...
- **인스타 = `임장로그`**. 전용 테마 `insta` 한 종 고정 — '앰버 포스터'안
  (bg #FFFFFF · 패널 #FFF7E7 · 앰버 #D97706/#F59E0B · 잉크 #141210 · Gothic A1 900 자간 −0.035em ·
  본문 Noto Sans KR · 라운드 0 · `.top`/`.trace`를 검은 3px 선으로).
  **2026-08-29 검정(터미널 데이터) → 밝은 판으로 전환**(사용자 요청). 앰버는 그대로 유지해
  기존 게시물·릴스와 이어진다. 앰버 글자는 흰 바탕에서 대비 3.5:1이라 **큰 숫자·제목에만** 쓴다.
  - 후보 비교: `themes/carousel-light.html`(밝은 5안 + 이전 검정, 리포트 #42 실물) ·
    `themes/carousel-samples.html`(2026-08-26 어두운 5안, 보관).
  - `make_cards.INSTA`에 후보 팔레트(`insta-dark`·`insta-ivory`·`insta-mint`·`insta-blueprint`·
