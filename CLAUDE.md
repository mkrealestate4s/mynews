...
## 주제 배분 (2026-09-06 사용자 지시 — 매일 주제 고를 때 먼저 본다)
사용자 지적: "매번 내용이 비슷해서 딱히 주목을 못 받는 것 같다. 송파 재건축 이슈 같은 건 없나."
최근 20편을 세어 보니 **14편(70%)이 통계·지수 해설**이었고 **단지 이름이 제목에 들어간 글은
#49(청담르엘) 하나뿐**이었다. "9월 입주 1만4,174가구"는 아무도 검색하지 않는 문장이다.
사람들은 **"잠실주공5단지" "장미아파트 시공사" "송파 재건축"**으로 검색한다.

- **주 7편 중 3편은 단지·구역 뉴스로 쓴다.** 제목에 **고유명사(단지명·구역명·조합명)**가
  반드시 들어가야 한다. 재건축·재개발 단지, 신규 분양 단지, 특정 구역이 대상이다.
  통계·지수 해설은 **주 4편 이하**로 줄인다.
- **지역 범위는 강남3구(송파·강남·서초) + 강북 주요구**(성북·노원·중랑·강북·도봉 등
  최근 오르는 구). 검색량과 상담 수요가 둘 다 있는 범위다.
- 단지 뉴스는 **일정표가 핵심**이다. 시공사 입찰공고·총회·심의·분양신청처럼 **날짜가 있는
  이벤트**를 앞에 놓는다. 규모(세대수·층수)는 그다음이다. 통계 해설과 달리 시의성이 짧으므로
  "이번 달에 무엇이 열리나"를 반드시 한 섹션으로 뺀다.
- 단지 뉴스에서 **단정하면 안 되는 것 3가지**: ① 계획 세대수·층수는 심의 단계마다 바뀌므로
  "확정 전"임을 캡션에 밝힌다(장미1·2·3차는 5,105 → 5,129로 바뀌었다). ② "어느 건설사가
  유력하다"는 **관측**이지 입찰 결과가 아니다. ③ 조합원 지위 양도·거래 제한은 단지·단계마다
  다르므로 일반론으로 답하지 않고 **조합·구청·정비사업 정보몽땅(cleanup.seoul.go.kr)** 확인으로
  넘긴다. 첫 편은 #52(송파 재건축, 2026-09-06)다.

### 배분보다 시의성이 먼저다 (2026-09-19 사용자 지적)
사용자: **"오늘 포스팅이 4월달 내용인데 이걸 오늘 올리는게 맞아?"** #65(신반포2차)의 핵심
사건인 통합심의 조건부 가결은 **2026-04-16**, 다섯 달 전 일이었다. 그날 단지 뉴스에서
신선한 건을 못 찾자 "주 3편은 단지 뉴스" 배분을 맞추려고 오래된 마일스톤을 끌어왔다.
같은 날 **국토부 토지거래허가구역 실거주 유예 연장 발표(9/17~18, 10/1 시행)**가 있었는데
그걸 놓쳤다.

- **배분은 주제를 고르는 기준이지 낡은 걸 올리는 근거가 아니다.** 순서를 뒤집지 말 것.
- **주제를 확정하기 전에 핵심 사건의 날짜를 적어 보고 오늘과 며칠 차이인지 센다.**
  대략 **1주일**을 넘으면 그건 오늘 뉴스가 아니다. gen 스크립트 docstring의 "수치와 검산"
  블록 맨 위에 **사건일과 오늘의 간격**을 한 줄로 적는다.
- 낡은 사건을 꼭 써야 하면 **그날 새로 생긴 것**(마감일·다음 단계·새 수치)을 리드로 올리고
  과거 사건은 배경으로 내린다. 리드가 과거면 그 글은 오늘 글이 아니다.
- **그날 단지 뉴스가 없으면 통계·제도로 간다.** 배분은 주 단위 목표지 하루 할당량이 아니다.
  못 채운 주가 있어도 낡은 글보다 낫다.

### 통계만 있는 글은 쓰지 않는다 (2026-09-26 사용자 지적)
사용자: **"이런내용 조회수 못먹어"** (#72 서울 85주 연속 상승 · #73 서울 전세 0.19%).
두 편 다 부동산원 주간 통계 해설이었고 제목에 단지·구역 이름이 없었다. 같은 날 사용자가
직접 고른 주제는 **"SH 장기전세 청담르엘"**(→ #74 특집)이었다. 검색되는 건 고유명사다.

- **주제 우선순위: 청약·분양 단지 > 재건축·재개발 일정 > 마감이 걸린 제도 > 통계.**
  통계는 그날 위 셋이 정말 없을 때만 쓰고, 그때도 **단지·구역 사례를 붙여** 제목에 고유명사를 넣는다.
- **title.txt 1번 제목에는 사람들이 검색창에 치는 말**(단지명·공고명·제도명)이 들어가야 한다.
  "서울 전세 0.19% 상승" 같은 지수 문장을 제목 앞머리에 두지 않는다.
- 위 '배분보다 시의성이 먼저다'는 그대로 유효하다. 고유명사를 넣으려고 낡은 사건을 끌어오지 않는다.

## 데일리 리포트 작성 절차
1. WebSearch로 당일 부동산 이슈 검색 → 데이터가 풍부한 키워드 1개 선정.
   **주제를 고르기 전에 위 '주제 배분'을 먼저 본다.** 최근 3편이 전부 통계 해설이면
   그날은 단지·구역 뉴스를 찾는다.
   **단, 위 '배분보다 시의성이 먼저다'가 이 규칙보다 위에 있다.** 배분을 맞추려고
   지난 사건을 끌어오지 않는다. 사건일이 오늘에서 1주일 넘게 떨어져 있으면 버린다.
...
5. **제목 추천 파일**: 사용자 보고에 넣는 블로그 제목 3개를 `title.txt`에 **덮어쓰고**
   ...
   - **발행이 끝난 큐 항목은 `posted`로 내린다** (2026-09-05 정리). `fetch_queue()`는
     `designated`·`pending`만 후보로 보고 중복을 **slug**로 조회하는데, `fetch_job()`은
     **job_id(`<slug>-01`)**를 posted.log에 기록한다. 두 키가 달라서 당일 manifest로 발행한 글이
     큐에서는 영원히 pending으로 남는다. 8/21 이후 15편이 그렇게 쌓여 있었고, 큐 경로를 한 번
     돌리면 지난 글이 줄줄이 다시 올라갈 상태였다. **다음 날 designated를 내릴 때 pending이
   ...
   - 커밋·배포는 보통 07:13~07:26에 끝난다(로컬 프로그램은 07:40 이후 받아감). 크게 늦어지면
     사용자에게 알린다.
   - **07:40은 목표가 아니라 마감이다.** 이 시각까지 배포가 끝나지 않으면 프로그램은
     **어제 파일을 받아간다.** 그때 title.txt·manifest가 이미 발행한 글을 가리키고 있으면
     그 글이 한 번 더 올라간다. 2026-09-05에 실제로 났다: 9/4 오후에 특집 #49로 title.txt를
     덮어 둔 상태에서 #50 배포가 11:16에 끝나, 프로그램이 07:40에 #49를 다시 집어갔고
     사용자에게는 "자동 포스팅이 안 됐다 · title.txt가 갱신되지 않는다"로 보였다.
     - 그래서 **특집으로 title.txt를 오후에 덮은 날은, 다음 날 데일리를 07:40 전에 올리는 것이
       특히 중요하다.** 늦어질 것 같으면 먼저 알려 사용자가 손으로 처리할 시간을 준다.
     - 늦게 올린 날 보고에는 **"프로그램이 어제 것을 받아갔을 수 있으니 확인하라"**를 넣는다.
     - 참고: 갱신 이력은 `git log -- title.txt` + `git show <커밋>:title.txt`로 확인한다.
       8/21~9/4는 매일 07:17~07:23(KST)에 정상 갱신됐다. 캐시 문제가 아니었다.

## 현재 상태 메모 · PC 자동등록 멈춤 (2026-09-26 사용자 알림)
사용자: **"지금 PC에서 자동등록 프로세스가 진행이 안 되는 상태야."**
- 로컬 포스팅 프로그램이 title.txt·manifest를 받아가지 않는다고 본다. **네이버 발행은 사용자가
  손으로 한다.** 데일리 절차(title.txt·manifest·queue 갱신, 07:40 전 배포)는 **그대로 유지**한다.
  프로그램이 다시 돌 때 파일 형식이 그대로여야 하기 때문이다.
- 멈춘 동안에는 07:40 마감을 넘겨도 중복 발행이 나지 않는다. 그래도 늦추지 않는다(언제 재개될지 모른다).
- **큐 status를 추측으로 posted로 내리지 않는다.** 프로그램이 안 돌면 "07:40에 받아갔다"는 전제가
  틀린다. 사용자가 손으로 올렸다고 알려 준 글만 `dropped`로 내리고(기존 수동 발행 규칙),
  나머지는 `pending`으로 둔다. ※ 9/26 #74 등록 때 #73을 posted로 잘못 내렸다가 사용자 확인(미발행) 후 pending으로 되돌렸다.
- **재개할 때 먼저 볼 것:** queue.json에 `designated`·`pending`이 여러 개 쌓여 있으면 큐 경로가
  지난 글을 줄줄이 올린다. 재개 전에 사용자에게 어떤 글을 이미 손으로 올렸는지 확인하고 정리한다.
- 매일 보고에 한 줄: "PC 자동등록 멈춤 상태 · 오늘 글은 손으로 발행 필요". 사용자가 재개됐다고
  알려 주면 이 절을 지운다.
...
- **캐러셀은 탭한 순서로 배치된다.** 그리고 **폰 갤러리·인스타 사진 선택 화면은
  ...
  **2026-08-31 사용자 실기기(안드로이드)에서 순서 정상 확인.** 세 겹이 다 필요해서 넣은 것이
  아니라 어느 하나가 기기·앱 업데이트로 어긋나도 버티게 겹쳐 둔 것이다.
  **간격을 줄이거나 저장 순서를 되돌리지 말 것** — 이 조합이 실기기에서 검증된 상태다.
...
- **제도·규정 해석은 1차 출처를 못 읽었으면 단정하지 않는다** (2026-09-04 실수에서 나온 규칙).
  이 환경은 WebFetch가 막혀 검색 요약만 볼 수 있다. 검색 요약은 기사 작성자의 해석이 섞여
  있어서 규정 문구와 다를 수 있다. 그런 상태에서 "A하면 B가 된다"로 쓰면 틀린다.
  - 실제로 틀린 예: "청약통장을 전환하면 국민주택 회차가 0으로 초기화된다"고 두 편(#46·#49)에
    걸쳐 단정했는데, 공고문 원문은 **"인정 실적이 달라질 수 있으므로 순위(가입)확인서로
    확인하라"**고만 쓴다. 초기화를 확정할 문구는 없었다. 사용자가 물어서 잡혔다.
  - 그리고 원문에는 내가 놓친 더 중요한 조건이 있었다: **"자격·순위 산정은 공고일 기준,
    전환은 공고일 전일까지 완료해야 반영"**. 결론은 같아도 근거가 이쪽이 맞다.
  - 그래서 이렇게 쓴다: 확인된 것만 단정하고, 나머지는 **"확인 필요"** 또는 원문 인용으로
    처리한다. 개인별 값(청약 회차·소득·자산 인정액 등)은 **어디서 확인하는지**를 알려 준다
    (청약통장 순위(가입)확인서, 청약홈 등). 일반론으로 답하지 않는다.
  - 공고문·법령 PDF를 넘겨받으면 **그것이 정본이다.** 기사 요약과 다르면 원문을 따른다.
    (PDF 텍스트 추출: `pip install pypdf cffi` 후 pypdf. pdftotext는 이 컨테이너에 없다.)
- 차트 컴포넌트: 가로 막대(.chart/.bar/.rail/.fill), 세로 컬럼 시계열(.colchart/.cols/.col, 눈금
  ...
  막대 밖에 배치(.railval).
- **`.statrow` 의 `.big` 은 짧은 값 전용이다** (2026-09-06 실측). 2.6rem + `nowrap` 이라
  8~9자를 넣으면 390px에서 폭 343px를 넘겨 옆의 `.desc` 를 밀어내고 **가로 오버플로가 난다**
  (#52 일정 차트에 "관리처분계획 총회"를 넣어 38px 넘쳤다). `.big` 에는 **숫자·연도·짧은
  시점**("9월"·"하반기"·"68.3%"·"1만4,174가구")만 두고 사건 이름·설명은 `.desc` 로 내린다.
